...
export default function EditEventPage() {
    ...
    const { data: eventResponse, isLoading: isFetchingEvent } = useGetEventById(eventId);
    const { mutate, isPending: isUpdating } = useUpdateEvent(eventId);
    ...
    useEffect(() => {
        if (eventResponse?.data) {
            ...
            setTime(data.eventTime || "");
            setLocation(data.location || "");
            setLinkGrup(data.groupLink || "");
            setLocationLink(data.locationLink || "");
            setMapTag(data.mapTag || "");
        ...
        return (
            <AdminLayout>
                <div className="flex items-center justify-center min-h-[60vh]">
                    <div className="w-12 h-12 border-4 border-slate-200 border-t-goldAccent rounded-full animate-spin"></div>
                </div>
            </AdminLayout>
        );
    }

    console.log("🚀 ~ EditEventPage ~ thumbnailPreview:", thumbnailPreview)
    return (
        <AdminLayout>
            <form onSubmit={handleSubmit} noValidate className="max-w-7xl mx-auto pb-32 pt-4 px-4 sm:px-0">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-6 mb-8">
                    <div className="flex flex-wrap items-center gap-2 text-xs sm:text-sm font-medium text-slate-500">
                        ...
                        <HiChevronRight className="w-3 h-3 shrink-0" />
                        <span className="text-navyBlue font-semibold whitespace-nowrap">Edit Event</span>
                    </div>
                    ...
                    <div className="flex items-center gap-3 w-full sm:w-auto">
                        ...
                        <button type="submit" disabled={isUpdating} className="flex-1 sm:flex-none text-center px-8 py-2.5 bg-navyBlue text-white font-medium rounded-xl hover:bg-goldAccent hover:text-navyBlue transition-all shadow-md text-sm disabled:opacity-50">
                        ...
                        <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-6 sm:p-8">
                            ...
                            <div className="space-y-6">
                                <div className="space-y-2">
                                    ...
                                    <input type="text" value={title} onChange={(e) => setTitle(e.target.value)} placeholder="Contoh: Webinar Wealth Management" className={getInputStyle(errors.title)} />
                                ...
                                <div className="space-y-2">
                                    ...
                                    <textarea rows={6} value={description} onChange={(e) => setDescription(e.target.value)} placeholder="Tuliskan deskripsi lengkap mengenai event ini..." className={`${getInputStyle(errors.description)} resize-none`}></textarea>
                    ...
                    <div className="w-full lg:w-1/3 space-y-6">
                        ...
                        <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-6">
                            ...
                            {!thumbnailPreview ? (
                                ...
                                <div className="relative w-full aspect-video rounded-xl overflow-hidden shadow-sm group border border-slate-200 bg-slate-100">
                                    <Image
